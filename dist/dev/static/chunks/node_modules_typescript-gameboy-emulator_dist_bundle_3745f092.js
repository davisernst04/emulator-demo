...
"[project]/node_modules/typescript-gameboy-emulator/dist/bundle.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var gpu = {
    ...
    checkStat: ()=>{
        ...
        if (gpu.curline === gpu.raster) {
            stat |= 4;
            if (stat & 64) condition = true;
        } else {
            stat &= ~4;
        }
        if (stat & 32 && gpu.linemode === 2) condition = true;
        if (stat & 16 && gpu.linemode === 1) condition = true;
        if (stat & 8 && gpu.linemode === 0) condition = true;
        gpu.reg[1] = gpu.reg[1] & 248 | stat & 4 | gpu.linemode;
    ...
    updateoam: (addr, val1)=>{
        ...
        if (obj < 40) {
            switch(oam_addr & 3){
                case 0:
                    gpu.objdata[obj].y = val1 - 16;
                    break;
                case 1:
                    gpu.objdata[obj].x = val1 - 8;
                    break;
                case 2:
                    gpu.objdata[obj].tile = val1;
                    break;
                case 3:
                    gpu.objdata[obj].palette = val1 & 16 ? 1 : 0;
                    gpu.objdata[obj].xflip = val1 & 32 ? 1 : 0;
                    gpu.objdata[obj].yflip = val1 & 64 ? 1 : 0;
                    gpu.objdata[obj].prio = val1 & 128 ? 1 : 0;
                    break;
            }
        }
    },
    rb: (addr)=>{
        ...
        switch(gaddr){
            case 0:
                return (gpu.lcdon ? 128 : 0) | (gpu.wintilebase === 7168 ? 64 : 0) | (gpu.winon ? 32 : 0) | (gpu.bgtilebase === 0 ? 16 : 0) | (gpu.bgmapbase === 7168 ? 8 : 0) | (gpu.objsize ? 4 : 0) | (gpu.objon ? 2 : 0) | (gpu.bgon ? 1 : 0);
            case 1:
                return 128 | gpu.reg[1] & 120 | (gpu.curline === gpu.raster ? 4 : 0) | gpu.linemode;
            case 2:
                return gpu.yscrl;
            case 3:
                return gpu.xscrl;
    ...
    wb: (addr, val1)=>{
        ...
        switch(gaddr){
            case 0:
                ...
                gpu.wintilebase = val1 & 64 ? 7168 : 6144;
                gpu.winon = val1 & 32 ? 1 : 0;
                gpu.bgtilebase = val1 & 16 ? 0 : 2048;
                gpu.bgmapbase = val1 & 8 ? 7168 : 6144;
                gpu.objsize = val1 & 4 ? 1 : 0;
                gpu.objon = val1 & 2 ? 1 : 0;
                gpu.bgon = val1 & 1 ? 1 : 0;
                gpu.reg[0] = val1;
                break;
            case 1:
                gpu.reg[1] = gpu.reg[1] & 7 | val1 & 120;
                gpu.checkStat();
                break;
            case 2:
                gpu.yscrl = val1;
...
var MBC1 = class {
    ...
    constructor(rom2, ramSize, romBanks, ramBanks){
        this.rom = rom2;
...
var MMU = class {
        ...
        switch(addr & 61440){
            ...
            case 61440:
                if (addr >= 65024 && addr <= 65183) {
                    ...
                    if (addr === 65287) return this.tac | 248;
                    if (addr === 65295) return this.intf | 224;
   ...
   */ wb(addr, val1) {
        switch(addr & 61440){
            ...
            case 24576:
            case 28672:
                this.mbc.writeByte(addr, val1);
                break;
            // 0x8000 - 0x9FFF: VRAM
            case 32768:
            case 36864:
                gpu.vram[addr & 8191] = val1;
                gpu.updatetile(addr, val1);
                break;
            // 0xA000 - 0xBFFF: External RAM (ERAM)
            case 40960:
            case 45056:
            ...
            case 61440:
                if (addr >= 65024 && addr <= 65183) {
                    gpu.oam[addr & 255] = val1;
                    gpu.updateoam(addr, val1);
                } else if (addr >= 65184 && addr <= 65279) {
                    break;
                } else if (addr >= 65280 && addr <= 65407) {
                    if (addr === 65350) {
                        for(let i = 0; i < 160; i++){
                            const v = this.rb((val1 << 8) + i);
                            gpu.oam[i] = v;
                            gpu.updateoam(65024 + i, v);
                        }
                    } else if (addr === 65280) {
                        joypad.select = val1 & 48;
                    } else if (addr === 65281) {
                        this.serialData = val1 & 255;
                    } else if (addr === 65282) {
                        this.serialControl = val1 & 255;
                        ...
                        this.cgbPrepareSpeedSwitch = val1 & 1;
                    } else if (addr >= 65344 && addr <= 65359) {
                        gpu.wb(addr, val1);
                    } else if (addr === 65360 && val1 === 1) {
                        this.inbios = 0;
                    }
                } else if (addr >= 65408 && addr <= 65534) {
                    this.zram[addr & 127] = val1;
                } else if (addr === 65535) {
                    this.inte = val1;
                } else if (addr >= 57344 && addr <= 65023) {
                    this.wram[addr & 8191] = val1;
   ...
   * @returns The written value.
   */ ww(addr, val1) {
        this.wb(addr, val1 & 255);
        this.wb(addr + 1, val1 >> 8 & 255);
        return val1;
    }
};
var mmu = new MMU();
...
var cpu = {
    ...
    step: ()=>{
        ...
        if (cpu.interrupts()) {} else if (cpu.halt) {
            cpu.reg.m = 1;
        } else {
            const pc = cpu.reg.pc;
    ...
    ops: {
        ...
        POP_rr: (rr)=>{
            const val1 = mmu.rw(cpu.reg.sp);
            if (rr === "af") cpu.af = val1;
            else cpu[rr] = val1;
            cpu.reg.sp = cpu.reg.sp + 2 & 65535;
            cpu.reg.m = 3;
            cpu.reg.t = 12;
        ...
        ADD_a_r: (r)=>{
            let val1 = cpu.reg[r];
            let res = cpu.reg.a + val1;
            cpu.reg.f = 0;
            if (!(res & 255)) cpu.reg.f |= cpu.FLAGS.Z;
            if ((cpu.reg.a & 15) + (val1 & 15) > 15) cpu.reg.f |= cpu.FLAGS.H;
        ...
        ADC_a_r: (r)=>{
            ...
            let res = cpu.reg.a + val1 + carry;
            cpu.reg.f = 0;
            if (!(res & 255)) cpu.reg.f |= cpu.FLAGS.Z;
            if ((cpu.reg.a & 15) + (val1 & 15) + carry > 15) cpu.reg.f |= cpu.FLAGS.H;
            if (res > 255) cpu.reg.f |= cpu.FLAGS.C;
            cpu.reg.a = res & 255;
            cpu.reg.m = 1;
            cpu.reg.t = 4;
        ...
        SUB_a_r: (r)=>{
            ...
            if ((res & 255) === 0) cpu.reg.f |= cpu.FLAGS.Z;
            if ((cpu.reg.a & 15) - (val1 & 15) < 0) cpu.reg.f |= cpu.FLAGS.H;
        ...
        SBC_a_r: (r)=>{
            ...
            cpu.reg.f = cpu.FLAGS.N;
            if ((res & 255) === 0) cpu.reg.f |= cpu.FLAGS.Z;
            if ((cpu.reg.a & 15) - (val1 & 15) - carry < 0) cpu.reg.f |= cpu.FLAGS.H;
            if (res < 0) cpu.reg.f |= cpu.FLAGS.C;
            cpu.reg.a = res & 255;
            cpu.reg.m = 1;
            cpu.reg.t = 4;
        ...
        INC_r: (r)=>{
            ...
            if ((val1 & 15) === 15) cpu.reg.f |= cpu.FLAGS.H;
            cpu.reg[r] = res;
            cpu.reg.m = 1;
            cpu.reg.t = 4;
        },
        ...
        DEC_r: (r)=>{
            ...
            cpu.reg.f |= cpu.FLAGS.N;
            if (res === 0) cpu.reg.f |= cpu.FLAGS.Z;
            if ((val1 & 15) === 0) cpu.reg.f |= cpu.FLAGS.H;
            cpu.reg[r] = res;
            cpu.reg.m = 1;
            cpu.reg.t = 4;
        },
        ...
        ADD_hl_rr: (rr)=>{
            ...
            if ((cpu.hl & 4095) + (val1 & 4095) > 4095) cpu.reg.f |= cpu.FLAGS.H;
        ...
        ADD_sp_n: ()=>{
            ...
            cpu.reg.f = 0;
            if ((sp & 15) + (imm & 15) > 15) cpu.reg.f |= cpu.FLAGS.H;
            if ((sp & 255) + (imm & 255) > 255) cpu.reg.f |= cpu.FLAGS.C;
            cpu.reg.sp = result;
            cpu.reg.m = 4;
            cpu.reg.t = 16;
        },
        ...
        RLC: (r)=>{
            let val1 = cpu.reg[r];
            let carry = val1 & 128 ? 1 : 0;
            let res = (val1 << 1 | carry) & 255;
            cpu.reg.f = carry ? cpu.FLAGS.C : 0;
        ...
        RRC: (r)=>{
            let val1 = cpu.reg[r];
            let carry = val1 & 1 ? 1 : 0;
            let res = (val1 >> 1 | carry << 7) & 255;
            cpu.reg.f = carry ? cpu.FLAGS.C : 0;
            if (res === 0) cpu.reg.f |= cpu.FLAGS.Z;
            cpu.reg[r] = res;
            cpu.reg.m = 2;
        ...
        BIT: (b, r)=>{
            ...
            if (!(val1 >> b & 1)) cpu.reg.f |= cpu.FLAGS.Z;
            cpu.reg.m = 2;
            cpu.reg.t = 8;
        },
        BIT_hl: (b)=>{
        ...
        DAA: ()=>{
            ...
            if (!(cpu.reg.f & cpu.FLAGS.N)) {
                if (cpu.reg.f & cpu.FLAGS.H || (a & 15) > 9) a += 6;
                if (cpu.reg.f & cpu.FLAGS.C || a > 159) {
                    a += 96;
                    cpu.reg.f |= cpu.FLAGS.C;
                }
            } else {
                if (cpu.reg.f & cpu.FLAGS.H) a = a - 6 & 255;
                if (cpu.reg.f & cpu.FLAGS.C) a -= 96;
            }
            cpu.reg.a = a & 255;
...
var parseHeader = (rom2)=>{
    let title = "";
    for(let i = 308; i <= 323; i++){
        if (rom2[i] === 0) break;
        title += String.fromCharCode(rom2[i]);
    }
    const type = rom2[327];
    const typeName = CARTRIDGE_TYPE_MAP[type] || "UNKNOWN";
    const romSizeCode = rom2[328];
    const ramSizeCode = rom2[329];
    ...
    for(let i = 308; i <= 332; i++){
        checksum = checksum - rom2[i] - 1 & 255;
    }
    const headerChecksum = rom2[333];
    const isHeaderChecksumValid = checksum === headerChecksum;
    let gChecksum = 0;
    for(let i = 0; i < rom2.length; i++){
        if (i !== 334 && i !== 335) {
            gChecksum = gChecksum + rom2[i] & 65535;
        }
    }
    const globalChecksum = rom2[334] << 8 | rom2[335];
    const isGlobalChecksumValid = gChecksum === globalChecksum;
    let isLogoValid = true;
    for(let i = 0; i < NINTENDO_LOGO.length; i++){
        if (rom2[260 + i] !== NINTENDO_LOGO[i]) {
            isLogoValid = false;
            break;
        }
    }
...
var createMBC = (rom2, type, ramSize, romBanks, ramBanks)=>{
    switch(type){
        case 0:
        // ROM ONLY
        case 8:
        // ROM+RAM
        case 9:
            return new MBC0(rom2);
        case 1:
        // MBC1
        case 2:
        // MBC1+RAM
        case 3:
            return new MBC1(rom2, ramSize, romBanks, ramBanks);
        case 15:
        // MBC3+TIMER+BATTERY
        case 16:
        ...
        // MBC3+RAM
        case 19:
            return new MBC3(rom2, ramSize, romBanks, ramBanks);
        case 25:
        // MBC5
        case 26:
        // MBC5+RAM
        ...
        // MBC5+RUMBLE+RAM
        case 30:
            return new MBC5(rom2, ramSize, romBanks, ramBanks);
        default:
...
var emulator = {
    ...
    loadRom: async (source2)=>{
        try {
            log.out("EMU", `Loading ROM from ${typeof source2 === "string" ? source2 : "Uint8Array"}...`);
            const cartridge2 = await loadRom(source2);
            mmu.setMBC(cartridge2.mbc);
            log.out("EMU", `ROM loaded: ${cartridge2.rom.length} bytes.`);
            return true;
        } catch (error) {
    ...
    run: async (source2)=>{
        ...
        emulator.init();
        const loaded = await emulator.loadRom(source2);
        if (loaded) {
...
//# sourceMappingURL=node_modules_typescript-gameboy-emulator_dist_bundle_3745f092.js.map